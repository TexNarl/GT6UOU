...
public final class SurfaceDepositWorldgen {
    ...
    public static int generate(WorldGenLevel level, ChunkAccess chunk, BlockPos.MutableBlockPos pos) {
        ...
        return generate(level, chunk, pos, null);
    }

    /** Uses Earth's real river/profile map while preserving all deposit rules. */
    public static int generateEarth(
            WorldGenLevel level,
            ChunkAccess chunk,
            BlockPos.MutableBlockPos pos,
            EarthGeneratorState earthState
    ) {
        if (!level.getLevel().dimension().equals(DimensionList.EARTH)) {
            return 0;
        }

        return generate(level, chunk, pos, earthState);
    }
    ...
    private static int generate(
            ...
            ChunkAccess chunk,
            BlockPos.MutableBlockPos pos,
            @Nullable EarthGeneratorState earthState
    ) {
        ...
        generatedBlocks += generateBlackSand(level, chunk, pos, minX, minZ, sandReplacementState, redSandReplacementState, earthState);
    ...
    private static int placeBlackSandVeinSlice(
        ...
        for (int localX = 0; localX < 16; localX++) {
            ...
            for (int localZ = 0; localZ < 16; localZ++) {
                ...
                if (!vein.contains(x, z) || !isValidBlackSandPlacement(level, x, z, earthState)) {
                    continue;
                }
    ...
    private static boolean isValidBlackSandPlacement(
            WorldGenLevel level,
            int x,
            int z,
            @Nullable EarthGeneratorState earthState
        ...
        if (earthState != null) {
            TerrainProfile profile = earthState.terrainProfiles().profileAtBlock(x, z);
            if (profile == TerrainProfile.OCEAN
                    || profile == TerrainProfile.DEEP_OCEAN
                    || EarthShoreProfiles.isShore(profile)) {
                return false;
            }

            var river = earthState.riverSampler().sample(x, z);
            return river != null && river.normalizedDistanceSq() <= 1.0;
        }
