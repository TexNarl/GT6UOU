package gregtech.worldgen.geology;
...
/**
 * Samples TFC-style geological rock stacks for the Overworld and GT Earth.
 *
...
public final class StoneLayerStackSampler {
    ...
    private static final Map<RegionType, List<StoneLayerStackDefinition>> WEIGHTED_STACKS = createWeightedStacks();
    ...
    @Nullable
    private final GeologicalProvinceResolver geologicalProvinceResolver;
    private final ThreadLocal<ResolvedProvince> resolvedProvinceCache = new ThreadLocal<>();
    ...
    /**
     * Creates a sampler whose broad geological province can come from an
     * owning world generator instead of this class's legacy standalone noise.
     *
     * <p>The Overworld uses the one-argument constructor and therefore keeps
     * its established province distribution. Earth supplies its regional
     * semantic map here so terrain, ores and worldgen previews can share one
     * geological source of truth.</p>
     */
    public StoneLayerStackSampler(long seed, @Nullable GeologicalProvinceResolver geologicalProvinceResolver) {
        this.seed = seed;
        ...
        this.geologicalProvinceResolver = geologicalProvinceResolver;
    }
    ...
    public String sampleRockId(int x, int y, int z, int surfaceY) {
        ...
        RegionType regionType = resolveGeologicalRegionType(x, z);
    ...
    private RegionType resolveGeologicalRegionType(int x, int z) {
        if (geologicalProvinceResolver == null) {
            return selectGeologicalRegionType(x, z);
        }

        ResolvedProvince cached = resolvedProvinceCache.get();
        if (cached != null && cached.x() == x && cached.z() == z) {
            return cached.type();
        }

        RegionType type = geologicalProvinceResolver.resolve(x, z);
        if (type == null) {
            throw new IllegalStateException("Geological province resolver returned null at " + x + ", " + z);
        }
        resolvedProvinceCache.set(new ResolvedProvince(x, z, type));
        return type;
    }
    ...
    private List<String> selectRockStack(RegionType type, int x, int z) {
        List<StoneLayerStackDefinition> weightedStacks = WEIGHTED_STACKS.get(type);
        if (weightedStacks == null || weightedStacks.isEmpty()) {
            throw new IllegalStateException("No weighted stone layer stacks for type: " + type);
        }
    ...
    private static Map<RegionType, List<StoneLayerStackDefinition>> createWeightedStacks() {
        Map<RegionType, List<StoneLayerStackDefinition>> stacks = new EnumMap<>(RegionType.class);
        for (RegionType type : RegionType.values()) {
            stacks.put(type, new ArrayList<>());
        }
        for (StoneLayerStackDefinition stack : StoneLayerList.OVERWORLD_LAYER_STACKS) {
            List<StoneLayerStackDefinition> weighted = stacks.get(stack.type());
            for (int index = 0; index < stack.weight(); index++) {
                weighted.add(stack);
            }
        }
        stacks.replaceAll((type, values) -> List.copyOf(values));
    ...
    private record ResolvedProvince(int x, int z, RegionType type) {
    }

    @FunctionalInterface
    public interface GeologicalProvinceResolver {
        RegionType resolve(int blockX, int blockZ);
    }
